...
void euler19()
{
    ...
    for (year=1901;year<=2000;year++)
    {
        for(m=0;m<12;m++)
        {
            if ((m==1)&&(year%4==0)) month[m]=29;
            for (day=1;day<=month[m];day++)
            {
                if ((d_week == 7)&&(day == 1))
                {
                    count_sun = count_sun+1;
                    printf("month %d %d started on sunday\n%d sundays so far\n\n", m+1, year, count_sun);
                    
                }
                
                if (d_week == 7) d_week = 1;
                else d_week = d_week+1;
            }
            if ((m==1)&&(year%4==0)) month[m]=28;
        }
